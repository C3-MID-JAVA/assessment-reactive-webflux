...
class StrategyFactoryTest {
  ...
  @BeforeEach
  void setUp() {
    List<TransactionCostStrategy> strategies = List.of(
            new ATMDepositStrategy(),
            new DepositStrategy(),
            new OnlinePurchaseStrategy()
    );
  ...
  void shouldReturnCorrectStrategy() {
    Mono<TransactionCostStrategy> strategyMono = Mono.just(strategyFactory.getStrategy(TransactionType.ATM_DEPOSIT));

    strategyMono.map(strategy -> {
  ...
  void shouldThrowExceptionForUnsupportedType() {
    Mono<TransactionCostStrategy> strategyMono = Mono.just(strategyFactory.getStrategy(TransactionType.PHYSICAL_PURCHASE));
