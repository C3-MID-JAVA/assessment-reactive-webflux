...
public class AccountServiceImpl implements AccountService {
    ...
    public Mono<AccountResponseDto> createAccount(AccountRequestDto requestDTO) {
        return Mono.just(requestDTO)
                ...
                .filter(req -> req.getInitialBalance() != null && req.getInitialBalance().compareTo(BigDecimal.ZERO) > 0)
                .switchIfEmpty(Mono.error(new CustomException("Initial balance must be a positive value")))
                .flatMap(req -> accountRepository.findByAccountNumber(req.getNumber())
                        .flatMap(existing -> Mono.error(new CustomException("Account number already exists: " + req.getNumber())))
                        .then(Mono.just(req)))
                .map(req -> Account.create(null, req.getNumber(), req.getInitialBalance())) // Use factory method
    ...
    public Mono<AccountResponseDto> getAccountDetailsByNumber(String accountNumber) {
        return accountRepository.findByAccountNumber(accountNumber)
                .map(AccountMapper::toResponseDTO)
                .switchIfEmpty(Mono.error(new CustomException("Account not found with number: " + accountNumber)));
    }
}
